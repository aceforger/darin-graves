  ...
  return (
    <section id="books" className="py-24 bg-[#09090A] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        {book.map((book, i) => (
            ...
            <div className="bg-[#16161A] border border-[#FF4500]/10 hover:border-[#FF4500]/30 transition-all shadow-lg hover:shadow-xl hover-lift h-full flex flex-col">
                {/* Book Cover - Auto size to image */}
                <div className="bg-gradient-to-br from-[#16161A] to-[#09090A] overflow-hidden relative flex items-center justify-center">
                {/* Status Badge */}
                <div className={`absolute top-1 left-1 z-10 px-2.5 py-1 font-rajdhani font-bold text-[9px] uppercase tracking-wider ${
                ...
                <img
                    ...
                    e.target.parentElement.innerHTML = `
                        ...
                        <div class="text-4xl font-orbitron font-bold text-[#FF4500] mb-2 tracking-tighter">${book.title.split(' ').map(w => w[0]).join('')}</div>
                        <p class="text-xs font-rajdhani font-bold text-[#E2E8F0] uppercase tracking-wider">${book.title}</p>
                        </div>
                    `
                    }}
                />
                ...
                {book.subtitle && (
                    <p className="text-[#64D28D] text-[9px] font-rajdhani font-bold uppercase tracking-[0.15em] mb-1.5">{book.subtitle}</p>
                )}
                <h3 className="text-base font-orbitron font-bold text-[#E2E8F0] mb-1.5 tracking-tight group-hover:text-[#FF4500] transition-colors">
                    {book.title}
                </h3>
                <p className="text-[#E2E8F0]/40 text-[11px] font-inter leading-relaxed mb-3 flex-1 line-clamp-2">
                    {book.description}
                </p>
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                    ...
                    <span key={i} className="px-2 py-0.5 bg-[#09090A] text-[#64D28D] text-[8px] font-rajdhani font-bold uppercase tracking-wider border border-[#64D28D]/10">
                        {theme}
                    </span>
                    ))}
                </div>
                ...
                <button className="w-full py-2.5 border border-[#FF4500]/30 text-[#FF4500] font-rajdhani font-bold text-[10px] uppercase tracking-[0.15em] hover:bg-[#FF4500]/10 transition-all group-hover:border-[#FF4500]/50">
                    View Details
                </button>
                </div>
            </div>
      ...
      {selectedBook && (
        ...
        >
          <div 
            className="bg-[#16161A] max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-[#FF4500]/20"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="flex items-center justify-between p-6 border-b border-[#FF4500]/10">
